...
React management UI, Chrome MV3 side panel/field adapter, and Windows native-messaging companion for `answer_cache_agent` 0.1. All application code/tooling here is independent of Fable's working files. No hosted application or telemetry.
...
Python checks require the existing engine dependencies. `test:host` configures only the command's Python import search path; it does not install/edit the engine. Test databases use temporary directories. `npm run contracts` reads existing schemas and writes only local generated types/snapshots. Do not run the root schema exporter.
...
See [Fable handoff](docs/fable-handoff.md), [integration](docs/integration.md), [acceptance](docs/acceptance.md), and [store materials](docs/store-listing.md). Development builds are not signed or publicly released products.
